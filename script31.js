...
let isSelectingBruceyDash = false;
...
class Unit extends Entity {
    constructor(x, y, radius, team, type) {
        ...
        if (type === 'bruce') {
            ...
            this.maxHp = 1380; this.hp = 1380; this.attackDamage = 345; this.speed = 50; this.color = '#8c7ae6';
        } else if (type === 'leon') {
    ...
    triggerDash(now) {
        if (this.type === 'brucey' && !this.hasDashed) {
    ...
    draw(ctx) {
        ...
        if (isSelectingBruceyDash && this.team === 'player' && this.type === 'brucey' && !this.hasDashed) {
            ctx.beginPath();
...
canvas.addEventListener('click', (e) => {
    ...
    if (isSelectingBruceyDash) {
        let clickedBrucey = units.find(u => u.team === 'player' && u.type === 'brucey' && !u.hasDashed && Math.hypot(u.x - x, u.y - y) <= u.radius * 2);
        if (clickedBrucey) {
            clickedBrucey.triggerDash(performance.now());
            isSelectingBruceyDash = false;
            document.getElementById('brucey-dash-btn').style.backgroundColor = '#8c7ae6';
        }
...
function updateUI() {
    ...
    if (dashBtn) {
        let activeBruceys = units.filter(u => u.team === 'player' && u.type === 'brucey' && !u.isDead && !u.hasDashed);
        if (activeBruceys.length > 0) {
            dashBtn.style.display = 'block';
            dashBtn.style.opacity = '1';
        } else {
            dashBtn.style.display = 'none';
            isSelectingBruceyDash = false; // Reset if all died/dashed
...
document.getElementById('brucey-dash-btn').addEventListener('click', (e) => {
    e.stopPropagation();
    let activeBruceys = units.filter(u => u.team === 'player' && u.type === 'brucey' && !u.hasDashed);
    if (activeBruceys.length > 0) {
        isSelectingBruceyDash = !isSelectingBruceyDash;
        let btn = document.getElementById('brucey-dash-btn');
        if (isSelectingBruceyDash) {
