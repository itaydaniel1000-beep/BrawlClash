...
function aiUpdate(dt, now) {
    ...
    if (difficulty === 'easy') {
        ...
        if (safeUnderAttack) {
            ...
            if (threats.length > 0) {
                let t = threats[0];
                if (enemyElixir >= CARDS['bull'].cost && Math.random() > 0.3) {
                    spawnEntity(t.x, Math.max(80, t.y - 40), 'enemy', 'bull'); // Drop Bull directly in front to block
        ...
        // PRIORITY 4: BUILD THE OVERWHELMING DEATH-HUB
        if (enemyElixir >= 6) {
            ...
            // Step E: Send infinite Tanks and Assassins specifically from the hyper-buffed safe zone!
            if (maxes.length > 0 && enemyElixir >= 5) {
                let p = pams[0];
                let spawnX = p.x;
                let spawnY = p.y + 60; // Front of the hub
                
                if (Math.random() > 0.5 && enemyElixir >= CARDS['leon'].cost) {
                    spawnEntity(spawnX + (Math.random()*80-40), spawnY, 'enemy', 'leon');
                    aiDelayTimer = now + 700; return;
                } else if (enemyElixir >= CARDS['bull'].cost) {
                    spawnEntity(spawnX + (Math.random()*80-40), Math.max(spawnY, hardAIAttackY), 'enemy', 'bull');
                    hardAIAttackY += 60; // Slowly push the spawnline forward to suffocate player
