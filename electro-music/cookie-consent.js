/* באנר הסכמה לעוגיות — עצמאי, מתאים לכל אתר.
   זוכר את הבחירה בעוגייה אמיתית (path=/, שנה) + localStorage כגיבוי.
   מפתח משותף — הסכמה פעם אחת חלה על כל האתרים באותו דומיין. */
(function () {
  ...
  style.textContent = `
    .cookie-banner {
      position: fixed; inset-inline: 16px; bottom: 16px; z-index: 2147483000;
      max-width: 760px; margin: 0 auto;
      background: #1c2238; border: 1px solid #2c3556; border-radius: 16px;
      box-shadow: 0 18px 50px rgba(0,0,0,.5); padding: 18px 20px;
      display: flex; align-items: center; gap: 16px; flex-wrap: wrap;
      color: #eef1ff; font-family: "Segoe UI","Heebo",system-ui,sans-serif;
      direction: rtl; animation: cookie-up .35s ease;
    }
    ...
    .cookie-banner .accept { background: linear-gradient(90deg, #6c8cff, #b06cff); color: #fff; }
    .cookie-banner .accept:hover { filter: brightness(1.1); }
    .cookie-banner .decline { background: rgba(255,255,255,.08); color: #9aa3c7; border: 1px solid #2c3556; }
  ...
  banner.innerHTML = '<div class="txt"><strong>🍪 אנחנו משתמשים בעוגיות</strong>' +
    '<p>האתר משתמש בעוגיות כדי לזכור את ההעדפות שלך ולשפר את החוויה.</p></div>' +
    '<div class="actions"><button class="decline">דחה</button><button class="accept">אני מסכים</button></div>';
