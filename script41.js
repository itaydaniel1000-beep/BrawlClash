...
const CARDS = {
    ...
    'bull': { name: 'בול', cost: 5, type: 'unit', color: '#341f97', icon: '🐂' },
    ...
    'max': { name: 'מהירות מקס', cost: 4, type: 'aura', color: '#f1c40f', icon: '⚡' },
    '8bit': { name: 'כוח 8-ביט', cost: 6, type: 'aura', color: '#e84393', icon: '🕹️' },
...
class Unit extends Entity {
    constructor(x, y, radius, team, type) {
        ...
        if (type === 'bruce') {
            ...
            this.maxHp = 1380; this.hp = 1380; this.attackDamage = 345; this.speed = 50; this.color = '#341f97';
        } else if (type === 'leon') {
    ...
    update(dt, now) {
        ...
        // Target acquisition
        if (this.type === 'bull') {
            // Bull attacks everything
            let enemies = units.concat(buildings, auras).concat([this.team === 'player' ? enemySafe : playerSafe]).filter(e => e.team !== this.team && !e.isInvisible && !e.isDead && !e.isFrozen);
...
class Building extends Entity {
    ...
    update(dt, now) {
        ...
        if (now - this.lastAttackTime > this.attackSpeed * atkSpeedMult) {
            // Find target
            let enemies = units.concat(buildings, auras).concat([this.team === 'player' ? enemySafe : playerSafe]).filter(e => e.team !== this.team && !e.isInvisible && !e.isFrozen);
...
class Projectile {
    ...
    hit() {
        ...
        if (this.isSplash) {
            // Explosion radius 80
            let enemies = units.concat(buildings, auras).concat([this.team === 'player' ? enemySafe : playerSafe]).filter(e => e.team !== this.team && !e.isFrozen);
            enemies.forEach(e => {
...
function buildDeck() {
    document.getElementById('deck-left').innerHTML = '';
    document.getElementById('deck-center').innerHTML = '';
    document.getElementById('deck-right').innerHTML = '';
    
    // Show side panels
    document.getElementById('deck-left').style.display = 'flex';
    document.getElementById('deck-right').style.display = 'flex';
    ...
    releaseBtn.onclick = (e) => {
        ...
        [...units, ...buildings, ...auras].forEach(ent => {
            if (ent.team === 'player' && ent.isFrozen) {
                ent.isFrozen = false;
            }
        });
    };
    ...
    for(let i=0; i<keys.length; i++) {
        ...
        let freezeBtn = document.createElement('div');
        freezeBtn.className = 'freeze-icon';
        freezeBtn.innerText = '❄️';
        freezeBtn.style.fontSize = '18px';
        freezeBtn.style.cursor = 'pointer';
        freezeBtn.style.background = '#74b9ff';
        freezeBtn.style.borderRadius = '50%';
        freezeBtn.style.width = '30px';
        freezeBtn.style.height = '30px';
        freezeBtn.style.display = 'flex';
        freezeBtn.style.alignItems = 'center';
        freezeBtn.style.justifyContent = 'center';
        freezeBtn.style.border = '2px solid white';
        freezeBtn.style.marginTop = '8px';
        freezeBtn.style.boxShadow = '0 2px #0984e3';
        
        freezeBtn.onclick = (e) => {
            e.stopPropagation();
            selectFreezeCard(cardKey, document.getElementById(`card-${cardKey}`));
        };
        ...
        cardContainer.appendChild(freezeBtn);
        
        d.onclick = () => selectCard(cardKey, d);
        
        if (card.type === 'aura') {
            document.getElementById('deck-right').appendChild(cardContainer); // Boosts on the right
        } else if (card.type === 'building') {
            document.getElementById('deck-left').appendChild(cardContainer); // Turrets on the left
            if (cardKey === 'scrappy') {
                // Penny is next in CARDS natively, so inserting releaseBtn here slots it explicitly between them!
                document.getElementById('deck-left').appendChild(releaseBtn);
            }
        } else {
            document.getElementById('deck-center').appendChild(cardContainer); // Walkers in the middle
        }
    }
}
...
canvas.addEventListener('click', (e) => {
    ...
    if (isSelectingBullDash) {
        ...
        if (clickedBull) {
            ...
            isSelectingBullDash = false;
            document.getElementById('bull-dash-btn').style.backgroundColor = '#8c7ae6';
        }
    ...
    if (selectedFreezeCardId) {
        ...
        if (!validPlacement) {
            let insideEmz = auras.some(a => a.team === 'player' && a.type === 'emz' && !a.isFrozen && Math.hypot(x - a.x, y - a.y) <= a.radius);
...
function spawnEntity(x, y, team, typeStr, isFrozen = false) {
    ...
    if (team === 'enemy') {
        ...
        if (!valid) {
            ...
            if (!insideEmz) y = CONFIG.CANVAS_HEIGHT / 2; // Snap strictly to river boundary
        }
    }
...
function update(dt, now) {
    if (playerElixir < playerMaxElixir) playerElixir += (CONFIG.ELIXIR_GEN_RATE * dt / 1000);
    let aiMaxElixir = difficulty === 'hard' ? 20 : CONFIG.MAX_ELIXIR;
    if (enemyElixir < aiMaxElixir) {
        let aiGenMult = difficulty === 'hard' ? 1.5 : 1.0;
        enemyElixir += (CONFIG.ELIXIR_GEN_RATE * aiGenMult * dt / 1000);
    }

    let oldEnemyCount = units.filter(u => u.team === 'enemy').length + buildings.filter(u => u.team === 'enemy').length + auras.filter(u => u.team === 'enemy').length;
    ...
    let newEnemyCount = units.filter(u => u.team === 'enemy').length + buildings.filter(u => u.team === 'enemy').length + auras.filter(u => u.team === 'enemy').length;
...
function aiUpdate(dt, now) {
    ...
    if (difficulty === 'easy') {
        ...
        let clusterY = 160;
        let pams = auras.filter(a => a.team === 'enemy' && a.type === 'pam');
        ...
        if (enemyElixir >= 4) {
            ...
            // Step E: Calculated Offense - Wait to bank massive elixir before striking!
            if (maxes.length > 0 && enemyElixir >= 16) {
                let p = pams[0];
                let spawnX = p.x;
                let spawnY = p.y + 60; // Front of the hub
                
                // If the player has no units, prefer Leon. Otherwise push.
                if (playerIsWeak && enemyElixir >= CARDS['leon'].cost) {
                    spawnEntity(spawnX + (Math.random()*80-40), spawnY, 'enemy', 'leon');
                    aiDelayTimer = now + 800; return;
                } else if (enemyElixir >= CARDS['bruce'].cost) {
                    let offChoice = Math.random();
                    let unitType = 'bruce'; // Default to basic Bear push
                    if (offChoice > 0.85 && enemyElixir >= CARDS['bull'].cost) unitType = 'bull'; // Rare Bull 
                    else if (offChoice > 0.5 && enemyElixir >= CARDS['leon'].cost) unitType = 'leon'; // Occasional Leon
                    
                    spawnEntity(spawnX + (Math.random()*80-40), Math.max(spawnY, hardAIAttackY), 'enemy', unitType);
                    hardAIAttackY += 60; // Slowly push the spawnline forward
                    if (hardAIAttackY > CONFIG.CANVAS_HEIGHT - 120) hardAIAttackY = clusterY + 60;
                    aiDelayTimer = now + 500; return;
                }
            }
        }
...
function updateUI() {
    elixirFill.style.width = `${Math.min(100, (playerElixir / playerMaxElixir) * 100)}%`;
    elixirText.innerText = `${Math.floor(playerElixir)} / ${playerMaxElixir}`;
    ...
    if (dashBtn) {
        ...
        if (activeBulls.length > 0) {
            ...
            isSelectingBullDash = false; // Reset if all died/dashed
            dashBtn.style.backgroundColor = '#8c7ae6';
        }
    }
    ...
    document.querySelectorAll('.card').forEach(c => {
        ...
        if (playerElixir < CARDS[cardKey].cost) {
            ...
            if (selectedFreezeCardId === cardKey) {
                selectedFreezeCardId = null;
                c.style.boxShadow = 'none';
            }
        } else {
