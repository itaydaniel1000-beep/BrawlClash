...
const CARDS = {
    ...
    'maxi': { name: 'מקסי', cost: 4, type: 'aura', color: '#fbc531', icon: '👟' },
...
class Safe extends Entity {
    ...
    update(dt, now) {
        if (now - this.lastAttackTime > CONFIG.SAFE_ATTACK_SPEED) {
            let target = this.findTargetInHalf();
            if (target) {
                projectiles.push(new Projectile(this.x, this.y, target, CONFIG.SAFE_DAMAGE, this.team, false));
...
class Unit extends Entity {
    ...
    update(dt, now) {
        ...
        let atkSpeedMult = 1;
        auras.forEach(a => {
            if (a.team === this.team && (a.type === 'max' || a.type === 'maxi') && Math.hypot(this.x - a.x, this.y - a.y) <= a.radius) {
                speedMult = 1.5; // 50% movement speed
                atkSpeedMult = 0.5; // ~50% attack speed equivalent delay multiplier
            }
        });

        // Find target: Safe or Emz
        let possibleTargets = [this.team === 'player' ? enemySafe : playerSafe];
        let enemyEmz = auras.filter(a => a.team !== this.team && a.type === 'emz' && !a.isDead);
        this.target = possibleTargets.concat(enemyEmz).sort((a,b) => Math.hypot(a.x - this.x, a.y - this.y) - Math.hypot(b.x - this.x, b.y - this.y))[0];
...
class Building extends Entity {
    ...
    update(dt, now) {
        ...
        auras.forEach(a => {
            if (a.team === this.team && (a.type === 'max' || a.type === 'maxi') && Math.hypot(this.x - a.x, this.y - a.y) <= a.radius) {
                atkSpeedMult = 0.5; // 50% faster attack speed modifier
            }
        });
        ...
        if (now - this.lastAttackTime > this.attackSpeed * atkSpeedMult) {
            ...
            if (inRange.length > 0) {
                ...
                projectiles.push(new Projectile(this.x, this.y, target, this.attackDamage, this.team, isSplash));
                this.lastAttackTime = now;
            }
        }
    }
...
class Aura extends Entity {
    ...
    draw(ctx) {
        ...
        // HP Bar only for Emz (legacy behavior)
        if (this.type === 'emz') {
...
function spawnEntity(x, y, team, typeStr) {
    ...
    if (card.type === 'aura') {
        auras.push(new Aura(x, y, team, typeStr));
    } else if (card.type === 'building') {
        buildings.push(new Building(x, y, team, typeStr));
    } else {
        units.push(new Unit(x, y, 15, team, typeStr));
    }
}
...
function update(dt, now) {
    ...
    if (enemyElixir < CONFIG.MAX_ELIXIR) enemyElixir += (CONFIG.ELIXIR_GEN_RATE * dt / 1000);
