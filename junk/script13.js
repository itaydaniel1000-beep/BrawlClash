...
class Unit extends Entity {
    ...
    update(dt, now) {
        ...
        // Find target: Safe or Auras
        let possibleTargets = [this.team === 'player' ? enemySafe : playerSafe];
        if (this.type !== 'bruce') {
            let enemyAuras = auras.filter(a => a.team !== this.team && !a.isDead);
            possibleTargets = possibleTargets.concat(enemyAuras);
        }
        this.target = possibleTargets.sort((a,b) => Math.hypot(a.x - this.x, a.y - this.y) - Math.hypot(b.x - this.x, b.y - this.y))[0];
...
function aiUpdate(dt, now) {
    ...
    for (let i = 0; i < pendingRebuilds.length; i++) {
        ...
        if (enemyElixir >= CARDS[rebuild.type].cost) {
            ...
            return; // STRICTLY block all other AI actions to save enough elixir for the repair!
        }
    }
