...
function aiUpdate(dt, now) {
    ...
    if (difficulty === 'easy') {
        ...
        let playerUnitsTanks = units.filter(u => u.team === 'player' && !u.isInvisible && (u.type === 'bruce' || u.type === 'bull' || u.type === 'leon'));
        let safeUnderAttack = (now - (enemySafe.lastDamageTime || 0) < 3000) || units.some(u => u.team === 'player' && !u.isInvisible && u.y < 250);
        let playerIsWeak = units.filter(u => u.team === 'player' && !u.isInvisible).length === 0 && buildings.filter(b => b.team === 'player' && !b.isInvisible).length === 0;
