...
function buildDeck() {
    ...
    for(let i=0; i<keys.length; i++) {
        ...
        if (card.type === 'aura') {
            document.getElementById('deck-right').appendChild(d); // Boosts on the right
        } else if (card.type === 'building') {
            document.getElementById('deck-left').appendChild(d); // Turrets on the left
        } else {
            document.getElementById('deck-center').appendChild(d); // Walkers in the middle
        }
    }
}
...
function aiUpdate(dt, now) {
    ...
    if (difficulty === 'easy') {
        ...
        // Ultimate Smart AI Strategy
        let playerPushers = units.filter(u => u.team === 'player' && (u.type === 'bruce' || u.type === 'bull' || u.type === 'leon'));
        let safeUnderAttack = (now - (enemySafe.lastDamageTime || 0) < 5000) || units.some(u => u.team === 'player' && u.target === enemySafe);
        
        // 1. Defend the Safe (Priority 1)
        if (safeUnderAttack && enemyElixir >= 4) {
            let defX = enemySafe.x + (Math.random() * 80 - 40);
            let defY = enemySafe.y + 60;
            if (enemyElixir >= 5 && Math.random() > 0.5) spawnEntity(defX, defY, 'enemy', 'bull');
            else spawnEntity(defX, defY, 'enemy', 'scrappy');
            aiDelayTimer = now + 400;
            return;
        }

        // 2. Counter-push incoming threats proactively (Priority 2)
        if (playerPushers.length > 0 && enemyElixir >= 4) {
            let threat = playerPushers.sort((a,b) => a.y - b.y)[0];
            if (threat.y < CONFIG.CANVAS_HEIGHT / 2 + 150) {
                if (enemyElixir >= 8 && !auras.some(a=>a.team==='enemy' && a.type==='pam' && Math.hypot(a.x-threat.x, a.y-threat.y)<150)) {
                    spawnEntity(threat.x, Math.max(70, threat.y - 120), 'enemy', 'pam');
                    aiDelayTimer = now + 500;
                    return;
                }
                if (enemyElixir >= 4) {
                    spawnEntity(threat.x + (Math.random()*60-30), Math.max(70, threat.y - 80), 'enemy', 'scrappy');
                    aiDelayTimer = now + 500;
                    return;
                }
            }
        ...
        // 3. Build an Overbearing Death Ball (Priority 3)
        if (enemyElixir >= 6) {
            let pushX = CONFIG.CANVAS_WIDTH / 2;
            let pushY = hardAIAttackY; 
            
            let hasPam = auras.some(a => a.team === 'enemy' && a.type === 'pam' && Math.hypot(a.x - pushX, a.y - pushY) < 120);
            let hasMax = auras.some(a => a.team === 'enemy' && a.type === 'max' && Math.hypot(a.x - pushX, a.y - pushY) < 120);
            let has8bit = auras.some(a => a.team === 'enemy' && a.type === '8bit' && Math.hypot(a.x - pushX, a.y - pushY) < 120);
            
            if (!hasPam && enemyElixir >= 8) {
                spawnEntity(pushX, pushY, 'enemy', 'pam');
                aiDelayTimer = now + 500; return;
            }
            if (!hasMax && enemyElixir >= 4) {
                spawnEntity(pushX - 60, pushY, 'enemy', 'max');
                aiDelayTimer = now + 500; return;
            }
            if (!has8bit && enemyElixir >= 6) {
                spawnEntity(pushX + 60, pushY, 'enemy', '8bit');
                aiDelayTimer = now + 500; return;
            }
            
            let localPennys = buildings.filter(b => b.team === 'enemy' && b.type === 'penny' && Math.hypot(b.x - pushX, b.y - pushY) < 200).length;
            if (localPennys < 3 && enemyElixir >= 5) {
                spawnEntity(pushX + (Math.random()*120-60), pushY + 40, 'enemy', 'penny');
                aiDelayTimer = now + 500; return;
            }
            
            if (enemyElixir >= 8 && Math.random() > 0.5) { 
                spawnEntity(pushX + (Math.random()*40-20), pushY + 60, 'enemy', 'leon');
                aiDelayTimer = now + 600;
                
                hardAIAttackY += 80; 
                if (hardAIAttackY > CONFIG.CANVAS_HEIGHT - 120) hardAIAttackY = 150; 
                return;
            }
            if (enemyElixir >= 5) {
                spawnEntity(pushX + (Math.random()*80-40), pushY + 60, 'enemy', 'bull');
