...
const GUIDE_CHAR_ROLES = {
    ...
    'spike':   { role: 'הילה — האטה + נזק', base: '1000 HP, ×0.5 מהירות + 200 dmg/sec', blurb: 'מאיט אויבים בטווח (×0.5 מהירות) **וגם** מוריד להם 200 חיים בכל שנייה כל עוד הם בתוך ההילה. הצירוף האטה+נזק הופך אותו לחזק במיוחד כי אויבים נשארים בפנים יותר זמן וסופגים יותר טיקים. SP1 מרפא גם יחידות שלך בתוך ההילה.' },
    'tara':    { role: 'הילה — משיכה + פיצוץ', base: '1500 HP, גרביטציה 150, 400 dmg על סיום', blurb: 'מושכת אויבים לתוכה — טובה לאיסוף גוש אויבים לטורטים שלך. **כשההילה נגמרת (אחרי 3 שניות) כל אויב שעדיין בתוכה מקבל 400 נזק חד-פעמי** — שילוב חזק עם ספייק שמאט אותם בפנים.' },
