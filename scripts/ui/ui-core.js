...
function updateStatsUI() {
    ...
    document.querySelectorAll('.resource-item.tokens .resource-value').forEach(el => el.innerText = formatNumber(playerStats.tokens || 0));
...
const GUIDE_CHAR_ROLES = {
    'bruce':   { role: 'יחידה — טנק', base: '1860 HP / 232 DMG, עלות 3', blurb: 'דוב חזק שסופג נזק והולם מכות כבדות. הולך אל הכספת ופוגע בה ישירות. מגבלה: עד 2 ברוסים פעילים בו-זמנית בכל צוות.' },
    'leon':    { role: 'יחידה — מתנקש', base: '900 HP / 200 DMG, מהיר ×1.3, עלות 8', blurb: 'נמצא במצב בלתי-נראה עד שהוא תוקף — היריב לא רואה אותו עד הרגע האחרון. SP1 (פאטשה) נותן מכת בונוס פי 2 במכה הראשונה ממצב בלתי-נראה.' },
    'bull':    { role: 'יחידה — מסתער', base: '1150 HP / 280 DMG, עלות 6', blurb: 'מסתער (דאש) לכיוון האויב הקרוב ביותר ונועל עליו. כפתור 💨 בצד מפעיל את הדאש; אחרי שהיעד מת אפשר ללחוץ שוב לדאש נוסף. SP2 נותן מגן של 500 חיים ל-5 שניות אחרי דאש.' },
    'scrappy': { role: 'בניין — טורט יריה', base: '800 HP / 60 DMG @150 טווח, כל 0.5ש׳, עלות 4', blurb: 'טורט שיורה במהירות על היחידה הקרובה. נהדר להגנה. SP1 (טעינה קופצת) — הכדור קופץ בין אויבים. SP2 — תיקון עצמי של 50 HP בשנייה.' },
    'penny':   { role: 'בניין — תותח ארוך-טווח', base: '600 HP / 200 DMG @299 טווח, כל 2.5ש׳, עלות 5', blurb: 'תותח עם טווח ארוך מאוד ונזק פיצוץ גדול. רכיב לחץ — שובר בניינים מרחוק. SP1 — פגזים מבעירים אזור. SP2 — יורה 4 פצצות כשהוא נהרס.' },
    'mr-p':    { role: 'בניין — מכונת זימון', base: '1000 HP, מזמן פורטרים כל 5ש׳, עלות 4', blurb: 'בסיס שמזמן פורטרים קטנים (100 HP / 50 DMG / מהירות 70) שתוקפים את הקו של היריב. SP1 (דלת מסתובבת) — מקפיץ זימון לכל 3ש׳. SP2 — לכל פורטר יש מגן 500 HP במכה הראשונה.' },
    'pam':     { role: 'הילה — מרפא', base: '700 HP, רדיוס 82, עלות 8', blurb: 'מרפאת יחידות שלך בתוך ההילה. SP1 (חיחיבוק) — ריפוי מיידי של 500 HP בהצבה. SP2 (לחץ של אמא) — פוגעת באויבים בהילה ב-20 dmg/sec.' },
    'max':     { role: 'הילה — מאיץ', base: '700 HP, רדיוס 82, מגביר ×0.5 מהירות התקפה, עלות 4', blurb: 'מאיץ את הצוות שלך בתוך ההילה — יחידות תוקפות פי 2 מהר. SP1 — מאיץ גם את מילוי האליקסיר ב-10%. SP2 — חסין להאטות.' },
    '8bit':    { role: 'הילה — מגביר נזק', base: '1200 HP, רדיוס 110, +10% נזק בהילה, עלות 6', blurb: 'מגביר נזק ליחידות הצוות בתוך ההילה. SP1 — טווח הילה +50%. SP2 — נזק +30% במקום 10%.' },
    'emz':     { role: 'הילה — הרחבת טריטוריה', base: '1000 HP, רדיוס 132, עלות 7', blurb: 'מרחיבה את האזור שבו אפשר להניח יחידות אל חצי האויב — חיים לפריצה. SP1 — אויבים בהילה סופגים +20% נזק. SP2 (הייפ) — ריפוי 30 HP לכל אויב בטווח.' },
    'spike':   { role: 'הילה — האטה + נזק', base: '1000 HP, רדיוס 55, ×0.5 מהירות + 200 dmg/sec, עלות 5', blurb: 'מאיט אויבים בטווח (×0.5 מהירות) **וגם** מוריד להם 200 חיים בכל שנייה כל עוד הם בתוך ההילה. הצירוף האטה+נזק הופך אותו לחזק במיוחד כי אויבים נשארים בפנים יותר זמן וסופגים יותר טיקים. SP1 (דשן) מרפא גם יחידות שלך בהילה. SP2 — זמן הילה ארוך יותר (15ש׳).' },
    'tara':    { role: 'הילה — משיכה + פיצוץ', base: '1500 HP, רדיוס 110, גרביטציה 150, 400 dmg על סיום, חיה 3ש׳, עלות 7', blurb: 'מושכת אויבים לתוכה — טובה לאיסוף גוש אויבים לטורטים שלך. **כשההילה נגמרת (אחרי 3 שניות) כל אויב שעדיין בתוכה מקבל 400 נזק חד-פעמי** — שילוב חזק עם ספייק שמאט אותם בפנים. SP1 — מזמן צל כשההילה נגמרת. SP2 — הגרביטציה מרפאת יחידות שלך.' },
    'amber':   { role: 'יחידה — שובל אש', base: '700 HP / 0 DMG, שביל 25 dmg/sec, מהירות 75, עלות 7', blurb: 'הולכת ומשאירה אחריה שביל אש שמוריד 25 חיים לשנייה לכל אויב שעובר בו. **חסינה לחלוטין** — לא ניתן להרוג אותה (נעלמת רק כשמגיעה ליעד). בזמן שמחזיקים את הקלף יש כפתור 🎯 — לחיצה עליו פותחת מצב מסלול: כל קליק על המפה מוסיף עד 6 נקודות (מקסימום 5 משבצות בין נקודות), ואחרי הקליק האחרון (או לחיצה שנייה על 🎯) אמבר נוצרת בנקודה הראשונה והולכת בין כולן ומשאירה שביל. בלי מסלול היא הולכת ליריב הקרוב ביותר.' },
    'bubble':  { role: 'יחידה — בועת מסטיק', base: '100 HP / 100 DMG מגע, מהירות ×6, עלות 3', blurb: 'באבל היא בועת מסטיק שעולה רק 3 אליקסיר. **אף דמות לא רואה אותה** — לא יורים בה, לא תוקפים, לא רודפים אחריה. כדי לשלוח אותה: **לחיצה ארוכה על המפה ומתיחה לכיוון** — היא תיורה כמו קלע באותו כיוון במהירות פי 6 מדמות רגילה. כל אויב שהיא עוברת עליו מקבל 100 נזק חד-פעמי (לא ניתן לפגוע באותו אויב פעמיים). מתנגשת בקיר → קופצת 90° לכיוון הנגדי. נכבית אחרי 36 צעדים של 50 פיקסל (סה"כ עד 1800 פיקסל).' },
    'sirius':  { role: 'ספל — שכפול', base: 'עלות דינמית: עלות-מטרה + 1', blurb: 'סיריוס היא ספל-שכפול. כשמרימים את הקלף, כל אויב על המגרש מסומן בעוטר סגול. **לחץ על אויב** כדי ליצור באותו מקום עותק טרי שלו בצוות שלך. העלות באליקסיר היא **עלות הדמות שלחצת + 1**. סיריוס עצמה לא מופיעה על המגרש. אי אפשר לשכפל פורטרים, שערים, שביל-אש, באבל, טרנק, או ספלים אחרים.' },
    'trunk':   { role: 'יחידה — תמיכה אנרגטית', base: 'בלתי-מנוצח, ללא מד-חיים, חי 15 שניות, עלות 5', blurb: 'טרנק עולה 5 אליקסיר ומשמש כיחידת תמיכה. הוא **הולך בעקראיות** רק בחצי שבו הצבת אותו ומפזר אחריו שביל אנרגיה סגול. **כשיחידה משלך דורכת על השביל**, הוא נעלם והיחידה מקבלת בונוס נזק קבוע של **+20%** (חד-פעמי, לא נערם, לא חל על פציפיסטים כמו אמבר). טרנק עצמו **בלתי-מנוצח** (לא מקבל נזק), אין לו מד-חיים גלוי, והוא **נעלם לבד אחרי 15 שניות**. דמויות-אויב לא רואות אותו ולא מנסות לתקוף אותו.' },
    'rosa':    { role: 'ספל — מגן', base: '500 HP מגן, –25 HP/שנייה, עלות 3', blurb: 'רוזה היא ספל הגנתי. כשמרימים את הקלף, **כל הדמויות שלך מסומנות בעוטר ורוד**. לחץ על דמות שלך כדי להעניק לה **בועת מגן** של 500 HP נוספים, שדועכים בקצב של **25 HP בכל שנייה**. המגן בולע נזק נכנס לפני ה-HP הרגיל. נערם על מגן קיים. רוזה עצמה לא מופיעה על המגרש. אי אפשר להעניק מגן לטרנק, באבל, או שבילים.' },
    'bonnie':  { role: 'בניין — צלף ארוך-טווח + טרנספורם', base: '700 HP / 250 DMG @450 טווח, כל 5ש׳, עלות 6', blurb: 'בוני היא טורט-צלף שעולה 6 אליקסיר. **טווח 450** — חצי מהמגרש — אבל אטית פי 2 מפני (5 שניות בין יריות). מתאימה לעמדה אחורית. **יכולת מיוחדת:** כפתור 🪄 בצד שמופיע כשיש בוני שלך על המגרש. לחיצה עליו מפעילה מצב טרנספורם, ואז קליק על בוני **הופך אותה לברוס מיוחד** (עם הילה זהובה) ש**תוקף הכל חוץ מהכספת** — מנקה את היריב מטורטים ויחידות. ההפיכה חינם.' }
};
